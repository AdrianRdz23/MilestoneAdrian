                    ...
                    <table class="table table-fixed">
                        <thead>
                            <tr>
                               ...
                               <th scope="col" class="col-3">Job</th>
                               <th scope="col" class="col-3">Education</th>
						...
						@foreach ($users as $u)
							...
							<input type="hidden" name="_token" value="{{ csrf_token()}}" /> <input
							   ...
							   <td class="col-3"><form action="job" method="post">
							<input type="hidden" name="_token" value="{{ csrf_token()}}" /> <input
								type="hidden" name="userid" value="{{ $u['ID'] }}" />
								 <input type="submit" class="btn btn-secondary btn-large" value="Job" />
								</form></td>
								
					           <td class="col-3"><form action="education" method="post">
							<input type="hidden" name="_token" value="{{ csrf_token()}}" /> <input
								type="hidden" name="userid" value="{{ $u['ID'] }}" />
								 <input type="submit" class="btn btn-secondary btn-large" value="Education" />
								</form></td>
